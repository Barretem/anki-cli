...
export class AnkiService {
  ...
  async findCardsDetail(query: string): Promise<Card[]> {
  ...
  async findCards(query: string): Promise<number[]> {
    return await this.invoke("findNotes", { query: `${query}` });
  }
...
let _ankiService: AnkiService;
let _initialised = false;

export function initAnkiService(ankiServiceUrl: string) {
  if (!_initialised) {
    _ankiService = new AnkiService(ankiServiceUrl);
    _initialised = true;
  }
}

export function getAnkiService(): AnkiService {
  if (!_initialised) {
    throw new Error("Anki Service not initialised");
  }
  return _ankiService;
}
