...
export class Serializer {
  ...
  private async splitByCards(mdString: string): Promise<ParsedData> {
    let rawCards = mdString
      .split(new RegExp(config["anki.md.card.separator"].default, "m"))
      .map((line) => line.trim());

    const deckName = this.deckName(rawCards);
    const noteType = config["anki.noteType"].default;
    const convertMath = config["anki.md.card.convertMath"].default;
    ...
    rawCards = rawCards.filter((str) => str.search(config["anki.md.deck.titleSeparator"].default as string) === -1);
  ...
  deckName(rawCards: string[]): string | null {
    const deckName = rawCards.reduce((acc, str) => {
      const match = str.match(new RegExp(config["anki.md.deck.titleSeparator"].default as string, "m"));
